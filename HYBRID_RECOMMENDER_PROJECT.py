...
movie = pd.read_csv("5.hafta Recommender_system/ödevler/datasets/movie.csv")
movie.head()
# UserID, film adı, filme verilen oy ve zaman bilgisini içeren veri seti
rating = pd.read_csv("5.hafta Recommender_system/ödevler/datasets/rating.csv")
rating.head()
...
movies_watched_df = user_movie_df[movie_watched]
...
suggested_movies = movies_to_be_recommend.merge(movie[["movieId", "title"]])
...
movie_id_of_user = df.loc[df["userId"] == user][df["rating"] == 5] \
...
user_movie_df.index == movie_id_of_user
selected_movie_title = movie.loc[movie["movieId"] == movie_id_of_user, "title"].values[0]

# Adım 4: Filtrelenen dataframe’i kullanarak seçili filmle diğer filmlerin korelasyonunu bulunuz ve sıralayınız.
selected_movie_title = user_movie_df[selected_movie_title]
user_movie_df.corrwith(selected_movie_title).sort_values(ascending=False).head(10)

# Adım 5: Seçili film’in kendisi haricinde ilk 5 film’I öneri olarak veriniz.
result = user_movie_df.corrwith(selected_movie_title).sort_values(ascending=False).head(10)
